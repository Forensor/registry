...
import Registry.Scripts.LegacyImport.Error (APIResource(..), ImportError(..), ManifestError(..), PackageFailures(..), RawPackageName(..), RawVersion(..), RequestError(..))
...
downloadLegacyRegistry = do
  ...
  releaseIndex <- Process.forPackage initialPackages \name repoUrl -> do
    ...
    let
      ...
      mkError = ResourceError <<< { resource: Left GitHubReleases, error: _ }
