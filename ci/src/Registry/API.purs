...
readOperation eventPath = do
  ...
  pure $ case Parser.jsonParser body of
    Left _err ->
      NotJson
...
runChecks metadata (Manifest manifest) = do
  ...
  Target libTarget <- case Map.lookup "lib" manifest.targets of
  ...
  let
    pkgNotInRegistry name = case lookupPackage name of
      Nothing -> Just name
      Just _p -> Nothing
  let pkgsNotInRegistry = Array.catMaybes $ map pkgNotInRegistry $ Set.toUnfoldable $ Map.keys libTarget.dependencies
